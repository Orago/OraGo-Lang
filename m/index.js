	...
	const flatLang = inputs => {
		...
		for (let section of handleSections(text)){
			...
			switch (method){
				...
				case 'log-data': console.log(langData); break;

				case 'print': console.log(...options); break;
				...
				case 'math':
					if (isMath(options[0]))
						curVal = evalMath(options[0]);
					
					console.log('Could Not Parse Math');
				break;

				case 'add':
					if (!options.map($ => isNaN($)).includes(true))
						curVal = options.map($ => Number($)).reduce((x, y) => x + y);
				break;
